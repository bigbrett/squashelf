...
static int comparePhdr(const void *a, const void *b) {
    ...
    if (pa->p_paddr < pb->p_paddr) return -1;
    if (pa->p_paddr > pb->p_paddr) return  1;
    return 0;
}
...
int main(int argCount, char **argValues) {
    ...
    const char *inputFile  = argValues[1];
    ...
    const char *outputFile = argValues[2];
    ...
    /*
     * Create a new ELF header in the output of the same class (32/64-bit).
     * This reserves space for the ELF header.
     */
    ...
    /*
     * Update the new ELF header:
     *  - Set program header count to loadCount
     *  - Remove all section headers (we only want PHT + loads)
     */
    ...
    /*
     * Copy each segment's file data in sorted order:
     *  - Align current file offset to p_align
     *  - Update p_offset in the PHT to point here
     *  - Read original segment bytes and write them to the new location
     */
    ...
    for (size_t i = 0; i < loadCount; i++) {
        ...
        /* Advance to end of this segment for next iteration */
        currentOffset = alignedOffset + seg.p_filesz;
    }

    /* Add one NULL section and point header at it */
    ...
    if (!nullScn) {
        ...
        /* Refresh and update ELF header to include this section */
        GElf_Ehdr outEhdr;
